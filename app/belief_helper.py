...
def validate(belief_text):
    ...
    if len(lines) < 4:
        return (
            f"Belief document seems incomplete. It needs to have the belief identifier, at least one claim, believer's identifier, and believer's signature on new lines - currently the document only has {len(lines)} lines.",
            None,
            None,
            None,
            None,
...
def get_belief(belief_id):
    ...
    try:
        presigned_url = minio_client.presigned_get_object(
            BELIEF_BUCKET, key, expires=timedelta(hours=2)
        )
